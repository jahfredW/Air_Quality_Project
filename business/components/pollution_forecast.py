...
class PollutionForecast:
    ...
    def init_pollution(self):
        ...
        self._pollution_ville = self._pollution_pyown._get_pollution_forecast(self.ville.nom)
        # si les données existes, on va créer une série d'objets de type pollution
        if self._pollution_ville is not None:
            for item in self._pollution_ville:

                for index in range(len(item)):
                    pollution = Pollution()
                    pollution.date = item[index][1]
                    pollution.ville = self.ville
                    pollution.aqi = item[index][2]['aqi']
                    pollution.co = item[index][2]['co']
                    pollution.no = item[index][2]['no']
                    pollution.no2 = item[index][2]['no2']
                    pollution.o3 = item[index][2]['o3']
                    pollution.so2 = item[index][2]['so2']
                    pollution.pm2_5 = item[index][2]['pm2_5']
                    pollution.pm10 = item[index][2]['pm10']
                    pollution.nh3 = item[index][2]['nh3']



                    self.pollutions[index] = pollution
                self.pollutions_liste.append(self.pollutions)
                self.pollutions = {}
